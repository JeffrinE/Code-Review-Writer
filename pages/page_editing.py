...
with st.sidebar:
    ...
    try:
        generated_content = llm_response_generator.editor_agent_response(st.session_state.generated_content, prompt, memory, st.session_state.llm)
        st.session_state.generated_content = generated_content
        container.write(st.session_state.generated_content)
        memory.append(generated_content)
    except Exception as e:
        container.warning(f"Error occured: "
                          f"{e}")
